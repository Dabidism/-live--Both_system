...
class ANPRService:
    ...
    def _handle_entry_exit_only(self, vehicle: Vehicle, is_entry: bool) -> None:
        ...
        if server_port == '5000':  # Entry system - only log entries
            ...
            # Create exit event for popup
            threading.Thread(target=self._create_exit_event, args=(vehicle.plate_num,), daemon=True).start()
    ...
    def _create_exit_event(self, plate_num: str) -> None:
        ...
        try:
            ...
            EventQueries.create_event(plate_num, 'exit', event_data)
            print(f"Created exit event for plate: {plate_num}")
        except Exception as e:
            print(f"Error creating exit event: {e}")
